		...
		foreach($adverts as $advert) {
			if(strlen($advert->text_ad_code) > 0) {
				...
				if($adrotate_config['clicktracking'] == 'Y') {
					$target_url = esc_attr($advert->link);
					$track_link = '';
				} else {
					$target_url = '%link%';
					$track_link = esc_attr($advert->link);
				}				
			...
			if($include_groups == 1) {
				$wpdb->insert($wpdb->prefix.'adrotate_linkmeta', array('ad' => $ad_id, 'group' => $group2zone[esc_attr($advert->zoneid)], 'block' => 0, 'user' => 0, 'schedule' => 0));
			}
	
			if($include_stats == 1) {
				$wpdb->insert($wpdb->prefix.'adrotate_stats', array('ad' => $ad_id, 'group' => 0, 'block' => 0, 'thetime' => $now, 'clicks' => esc_attr($advert->clicks), 'impressions' => esc_attr($advert->impressions)));
			}
