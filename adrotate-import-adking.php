		...
		foreach($adverts as $advert) {
			...
			if($meta_advert['akp_media_type'][0] == 'image' OR $meta_advert['akp_media_type'][0] == 'text' OR $meta_advert['akp_media_type'][0] == 'adsense') {
				...
				if($include_schedules == 1) {
					$wpdb->insert($wpdb->prefix.'adrotate_schedule', array('name' => 'Imported schedule for advert '.$ad_id, 'starttime' => $now, 'stoptime' => $meta_advert['akp_expiry_date'][0], 'maxclicks' => 0, 'maximpressions' => 0, 'spread' => 'N', 'hourimpressions' => 0));
